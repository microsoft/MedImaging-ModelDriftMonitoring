...
from callbacks import VAEPredictionWriter
...
parser.add_argument(
    "--model", type=str, dest="model", help="path to model or registered model name"
)
parser.add_argument(
...
parser.add_argument(
    "--append_run_name", type=int, dest="append_run_name", default=False
)
...
if args.run_azure:
    ...
    args.model = m.download(
        ...
        target_dir=os.path.join(
            args.output_dir, "checkpoint", f"{node_rank}-{local_rank}"
        ),
    )
