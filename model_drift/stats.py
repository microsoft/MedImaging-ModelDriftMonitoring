...
def calc_p_categorical(train, test):
    ...
    keys = set(
        list(train[np.isfinite(train)].unique())
    ...
    try:
        if len(keys) > 2:
            f_exp = np.array([value[1] for value in sorted(ref_feature_dict.items())])
            f_obs = np.array(
                [value[1] for value in sorted(current_feature_dict.items())]
            )
...
def calculate_alerts(df_roll, test):
    ...
    for edge in edges:
        ...
        drill_end = edge[1]
        df_drill = df_roll[
            (df_roll.index >= drill_start) & (df_roll.index <= drill_end)
        ]

        high = df_drill.max()
        low = df_drill.min()

        dfalert = pd.DataFrame(
            [
                (df_drill.index.min(), high, low),
                (df_drill.index.max(), high, low),
            ],
