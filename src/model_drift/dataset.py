...
class BaseDataset(Dataset):
    ...
    def read_image(self, image_path):
        try:
            ...
            print(f"\nbad path: {image_path}\n")
            image_data_original = Image.fromarray(np.zeros((self.image_size, self.image_size, 3), dtype=np.uint8))
...
class ChestXrayDataset(BaseDataset):
    def prepare_data(self):
        ...
        dataframe = pd.read_csv(os.path.join(self.folder_dir, self.dataframe_path), low_memory=False)
...
class PadChestDataset(BaseDataset):
    ...
    def prepare_data(self):
        ...
        if self.frontal_only:
            dataframe = dataframe[dataframe["is_frontal"].astype(bool)]
        if self.load_labels:
            dataframe["label"] = dataframe[self.label_cols].apply(list, axis=1)
        for index, row in dataframe.iterrows():
            ...
            # if len(row) < 10:
            labels = row["label"] if self.load_labels else [0] * len(self.label_cols)
            self.frontal.append(float(row["is_frontal"]))
            self.image_labels.append(labels)
