...
from model_drift.drift import Sampler, ChiSqDriftCalculator, KSDriftCalculator, TabularDriftCalculator
from model_drift.drift import BasicDriftCalculator, KdeIntersectionCalculator, NumericalHistIntersectionCalculator, HistIntersectionCalculator, KdeHistPlotCalculator
...
def auto_generate_metric_calculator(dataframe, vae_cols=r"mu\..*", score_cols=r"activation\..*", 
                                    metadata_cols=(r'.*_DICOM', r"^age", r"Exposure*"), exclude_cols=r"StudyDate_DICOM"):
    ...
    def add_score_metrics(dwc: TabularDriftCalculator, col: str):
        ...
        dwc.add_drift_stat(col, KdeHistPlotCalculator(npoints=100), drilldown=True, group="ai")
    ...
    def add_metadata_metrics(dwc: TabularDriftCalculator, col: str):
        if col in metadata_float_cols:
            ...
            dwc.add_drift_stat(col, KdeHistPlotCalculator(npoints=100), drilldown=True, group="metadata")
...
def padchest_default_config(dataframe, vae_cols=r"mu\..*", score_cols="activation\..*"):
    ...
    metadata_float_cols = ["WindowCenter_DICOM", "WindowWidth_DICOM", "Rows_DICOM", "Columns_DICOM", "ExposureInuAs_DICOM", "RelativeXRayExposure_DICOM"]
    metadata_cat_cols = ["Projection", "PatientSex_DICOM", "ViewPosition_DICOM", "Modality_DICOM", "Manufacturer_DICOM", "PhotometricInterpretation_DICOM", "PixelRepresentation_DICOM",
                         "PixelAspectRatio_DICOM", "SpatialResolution_DICOM", "BitsStored_DICOM", "XRayTubeCurrent_DICOM", "Exposure_DICOM"]
    ...
    def add_metadata_metrics(dwc: TabularDriftCalculator, col: str):
        if col in metadata_age_cols:
            dwc.add_drift_stat(col, KSDriftCalculator(), drilldown=False, group="metadata")
            dwc.add_drift_stat(col, KdeHistPlotCalculator(npoints=100, hist_tol=0, kde_tol=0), drilldown=True, group="metadata")
